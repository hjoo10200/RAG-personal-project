...
이 프로젝트에서 사용하는 `intfloat/multilingual-e5-small`은 하나의 문서 청크나 검색 질문을 384개의 실수로 변환한다. 따라서 Elasticsearch가 실제 단어의 포함 여부를 중심으로 찾는다면, 임베딩 검색은 문장 전체의 의미가 얼마나 가까운지를 중심으로 찾는다.

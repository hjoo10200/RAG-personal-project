...
이 문서는 청년 자취 독립 플래너의 실제 보고서 생성 경로에 적용된 Hybrid 검색을 설명한다. PGVector의 의미 검색과 Elasticsearch BM25 키워드 검색을 왜 함께 사용하는지, 서로 다른 점수를 어떻게 Weighted RRF로 결합하는지, 어떤 근거가 LangChain과 Groq LLM에 전달되는지 코드 기준으로 정리한다.
...
flowchart TD
    ...
    LC --> GROQ["Groq openai/gpt-oss-120b"]
    GROQ --> VALIDATE["출처·형식 검증"]
...
| `src/generation/report_generator.py` | LangChain과 Groq를 이용한 판단 중심 서술형 보고서 생성 |
...
점수 순서만 사용하면 한 PDF가 결과를 독점할 수 있다. 먼저 서로 다른 출처를 하나씩 선택하고, 남은 자리는 같은 PDF의 다른 페이지를 허용한다. 같은 `chunk_id`와 같은 출처·페이지의 반복 청크는 제외한다. 각 본문은 LLM 입력 크기와 Groq 요청 한도를 통제하기 위해 최대 400자로 정리한다.
...
이 파일을 먼저 확인하면 보고서 오류가 검색 때문인지 생성 때문인지 구분할 수 있다. `retrieval_methods`에 두 값이 있으면 양쪽 검색기가 같은 청크를 발견한 것이다. 이 점수와 하위 질의는 추적용 JSON에는 보존하지만 LLM에는 전달하지 않는다. 생성 모델에는 사용자 상황과 코퍼스·파일명·페이지 및 각 근거 본문의 앞 300자만 전달해 Groq 요청 토큰을 절약하고 검색 점수가 서술에 섞이는 것을 막는다. 추적 JSON에는 기존 400자 근거를 그대로 남긴다.

## 13. 보고서 생성

생성에는 LangChain의 `ChatPromptTemplate`, `ChatGroq`, `with_structured_output`과 Runnable 파이프라인을 사용한다. 모델은 Groq의 `openai/gpt-oss-120b`다.
...
보고서 문체는 전체를 `합니다/됩니다` 형식의 존댓말로 통일한다. 분석 문단은 사용자 입력 수치가 실제로 무엇을 의미하는지와 진행·조정 조건을 설명하고, 실행 문단은 먼저·계약 직전·이사 당일·입주 후 순서로 확인 대상과 결과에 따른 다음 행동을 서술한다. 근거 부족을 이유로 같은 확인 문장만 반복하지 않도록, 확인할 항목과 비교 대상까지 구체적으로 작성한다.

최종 출력은 별도의 복잡한 배열이 아니라 `report_title`과 하나의 `report_body_markdown`을 가진 JSON이다. 본문은 2,400자 이상의 존댓말 서술형 문단으로 작성된다. 예산은 적절성 판단의 보조 근거로만 사용하고, 집 찾기·계약·이사·정착 방법을 본문의 중심에 둔다.
...
- 다섯 개 이상의 소제목. 첫 절은 한 문단, 나머지는 각 두 개 이상의 문단
- 전체 본문 2,400자 이상
- 목록 대신 서술형 문단 사용
- 부동산·계약, 이사, 주의점, 지원정책 포함
- 실제 검색된 파일명·페이지만 출처로 사용
- 모든 출처 표시는 `[출처: 파일명, p.페이지]` 형식 준수
...
현재 시스템은 사용자의 구조화된 독립 상황을 의미 검색과 정확어 검색에 각각 맞는 하위 질의로 변환하고, PGVector와 Elasticsearch의 순위를 Weighted RRF로 결합하여 실제 PDF 근거를 선별한 다음, LangChain과 Groq LLM으로 독립 적절성·실행 과정·주의점·지원정책을 포함한 출처 기반 서술형 보고서를 생성한다.

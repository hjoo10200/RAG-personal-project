...
브라우저에서 `http://127.0.0.1:8765/`에 접속합니다. 기본값은 외부 검색·생성 금지입니다.

### 테스트 페이지: 실제 검색·보고서 허용

```powershell
.\.venv\Scripts\python.exe -m src.test_page --enable-external
```

DB와 Elasticsearch가 실행 중이고 기존 컬렉션/인덱스가 준비되어 있어야 합니다. `.env`의 OpenAI 설정도 필요합니다. 페이지를 여는 것만으로 보고서가 생성되지는 않으며 버튼을 누르면 실제 API 사용료가 발생할 수 있습니다. 서버 종료는 `Ctrl+C`; 변경 후에는 재시작하고 페이지를 새로고침합니다.
